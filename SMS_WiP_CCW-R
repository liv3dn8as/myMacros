...
Sub SMS_WIP_CCWR()
    ...
    Dim theQty, qtyRng As Range 'for selecting the ranges of the above sections
    ...
    Set theSO = Cells.Find(What:="Contract Number", LookAt:=xlWhole)
    ...
    'Delete rows that are blank in column I(SKU) to reduce working data
    With wrkngSht
    ...
    'Delete rows that are blank in column F(SKU) to remove Sample Invoice section
    With wrkngSht
        lastRow = .Cells(.Rows.Count, "A").End(xlUp).Row
        For i = lastRow To startRow Step -1
    ...
    Set theSK = wrkngSht.Cells.Find(What:="SKU", LookAt:=xlWhole, MatchCase:=True)
    Set skRng = wrkngSht.Range(theSK, theSK.End(xlDown))
    skRng.Cut

    wrkngSht.Range("C:C").Insert Shift:=xlToRight

    Set theBD = wrkngSht.Cells.Find(What:="Start Date", LookAt:=xlWhole, MatchCase:=True)
    Set bdRng = wrkngSht.Range(theBD.Offset(0, 1), theBD.End(xlDown))
    bdRng.Cut

    wrkngSht.Range("D1").Insert Shift:=xlToRight

    Set thePR = wrkngSht.Cells.Find(What:="Extended Net Price", LookAt:=xlWhole, MatchCase:=True)
    Set prRng = wrkngSht.Range(thePR, thePR.End(xlDown))
    prRng.Cut

    wrkngSht.Range("F1").Insert Shift:=xlToRight
    
    Set theQty = wrkngSht.Cells.Find(What:="Quantity", LookAt:=xlWhole, MatchCase:=True)
    Set qtyRng = wrkngSht.Range(theQty, theQty.End(xlDown))
    qtyRng.Cut
    
    wrkngSht.Range("G1").Insert Shift:=xlToRight
    ...
    'Insert Blank Rows After Each Change In SMARTnet(L) Level
    With wrkngSht
    For i = lr To 3 Step -1
        If Cells(i - 1, "L").Value <> Cells(i, "L").Value Then
            Cells(i, "L").Resize(3).EntireRow.Insert
    ...
    With orderSht
        ...
        .Range("B4").Value = "Part Number"
        .Range("C4").Value = "Quantity"
        .Range("B4:C4").Font.Bold = True
    End With
    
    Set bBox = orderSht.Range("B2:C2")
